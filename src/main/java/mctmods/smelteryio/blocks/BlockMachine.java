...
public class BlockMachine extends BlockBaseTE {
	...
	@SuppressWarnings("deprecation")
	@Override public float getBlockHardness(@Nonnull IBlockState blockState, @Nonnull World worldIn, @Nonnull BlockPos pos) { return blockState.getValue(VARIANT).getHardness(); }

	@Override public int getHarvestLevel(@Nonnull IBlockState state) { return state.getValue(VARIANT).getHarvestLevel(); }

	@Override public float getExplosionResistance(@Nonnull World world, @Nonnull BlockPos pos, @Nullable Entity entity, @Nonnull Explosion explosion) {
		IBlockState state = world.getBlockState(pos);
		return state.getValue(VARIANT).getResistance() * 3.0F / 5.0F;
	}
	...
	@Override public void breakBlock(World world, @Nonnull BlockPos pos, @Nonnull IBlockState state) {
		TileEntity tileEntity = world.getTileEntity(pos);
		if (tileEntity != null) {
			if (tileEntity instanceof TileEntityFC && ((TileEntityFC) tileEntity).getMasterTile() != null) {
				((TileEntityFC) tileEntity).resetSmeltery();
			}
			if (((TileEntityBase) tileEntity).getMasterTile() != null) {
				((TileEntityBase) tileEntity).notifyMasterOfChange();
			}
	...
	@Override public void randomDisplayTick(@Nonnull IBlockState state, World world, @Nonnull BlockPos pos, @Nonnull Random rand) {
		TileEntity tileEntity = world.getTileEntity(pos);
