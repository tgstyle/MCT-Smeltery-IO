		...
		switch (tileID) {
			...
			case 0:
				...
				if (slot == TileEntityFC.SLOTUPGRADESPEED) {
					for (EnumUpgrade type : allowSpeedList) {
						if (stack.getItem() == Registry.UPGRADE && stack.getMetadata() == type.ordinal()) {
							return true;
						}
					}
				}
			...
			case 1:
				if (slot == TileEntityCM.SLOTFUEL) {
					return stack.getItem() == Items.SNOWBALL || stack.getItem() == Registry.ICEBALL;
				}
				if (slot == TileEntityCM.SLOTCAST) {
					return stack.getItem() != Registry.UPGRADE
							&& stack.getItem() != Items.SNOWBALL
							&& stack.getItem() != Registry.ICEBALL;
				}
