...
public class GuiFC extends GuiContainer {
	...
	@Override protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		String temperatureInfo = TextFormatting.RED + I18n.format("container.fuel_controller.temperature");
		...
		String msgRatio = TextFormatting.AQUA + I18n.format("container.fuel_controller.ratio") + " " + String.format("%.1f", ratio);
		fontRenderer.drawString(msgRatio, -55 - fontRenderer.getStringWidth(msgRatio) / 2, 32, 4210752);

		String warn = null;
		if (!tileEntity.isHeatingSmeltery()) { warn = TextFormatting.DARK_RED + I18n.format("container.fuel_controller.erroritems"); }
		if (tileEntity.atCapacity()) { warn = TextFormatting.DARK_RED + I18n.format("container.fuel_controller.errorcapacity"); }
		if (tileEntity.isReady()) { warn = TextFormatting.DARK_RED + I18n.format("container.fuel_controller.errorfuel"); }
		if (!tileEntity.isFueled()) { warn = TextFormatting.DARK_RED + I18n.format("container.fuel_controller.errorsmelteryfuel"); }
		if (!tileEntity.getOwner()) { warn = TextFormatting.DARK_RED + I18n.format("container.fuel_controller.errorowner"); }
		if (!tileEntity.hasController()) { warn = TextFormatting.DARK_RED + I18n.format("container.fuel_controller.errorsmeltery"); }
